...
router.post("/borrow", async (req, res) => {
    try {
        ...
        const { borrower_emp_id, owner_emp_id, itemId, quantity, DPT_ID } = req.body;

        if (!borrower_emp_id || !owner_emp_id || !itemId || quantity === undefined || quantity === null || !DPT_ID) {
        ...
        console.log(`🔄 Processing borrow request: Borrower ${borrower_emp_id}, Item ${itemId}`);
        ...
        // Fetch item details (✅ FIXED: Searching using ITEM_ID now)
        const [itemResult] = await db.query(
            `SELECT ITEM_ID, name, description, quantity, accountable_emp, ics, are_no, prop_no, serial_no, pis_no, class_no 
             FROM distributed_items WHERE ITEM_ID = ?`,  // 🔥 FIXED: Changed "id" to "ITEM_ID"
            [itemId]
        );

        if (itemResult.length === 0) {
            console.error(`❌ Item with ITEM_ID=${itemId} not found in distributed_items table`);
        ...
        // Insert borrow transaction (✅ FIXED: Passing ITEM_ID instead of id)
        const [insertResult] = await db.query(
            ...
            [itemId, borrower_emp_id, owner_emp_id, quantity, DPT_ID]  // ✅ FIXED: Using ITEM_ID
        );
