...
// filename: lib/server/distributed_items.js
...
// Get distributed_items only if emp_id matches accountable_emp
...
// Fetch distributed_items based on department ID
router.get("/department/:currentDptId", async (req, res) => {
    ...
    try {
        ...
        const query = "SELECT * FROM distributed_items WHERE current_dpt_id = ? AND deleted = 0";
