...
router.post("/lend_transaction", async (req, res) => {
    try {
        const { emp_id, distributedItemId, quantity, borrowerId, currentDptId } = req.body;
        console.log("📩 Received Lending Request:", req.body);

        if (!emp_id || !distributedItemId || !borrowerId || !quantity || !currentDptId) {
            return res.status(400).json({ message: "Missing required parameters" });
        }
        ...
        const [result] = await db.query(
            `INSERT INTO borrowing_transaction 
             (distributed_item_id, borrower_emp_id, owner_emp_id, quantity, status, createdAt, updatedAt, DPT_ID) 
             VALUES (?, ?, ?, ?, 2, NOW(), NOW(), ?)`,

            [distributedItemId, borrowerId, emp_id, quantity, currentDptId]
        );
